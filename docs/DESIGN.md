Design system for the **Landing Page** (`/`). Remix the design in `doc/Design_System.md` to match the visual identity in `docs/Product_Brief_Digital_Invitation_SaaS.md`. Apply its colors, typography, and logo usage while keeping the existing layout and structure.

Note: Dashboard (admin, customer, scanner) uses shadcn/ui + Tailwind CSS. Public invitation uses the design system from the admin's custom HTML/CSS/JS templates.
...
## Animation Guide
### Three.js animation
keyframe animation, skeletal animation, morph targets, animation mixing. Use when animating objects, playing GLTF animations, creating procedural motion, or blending animations.
